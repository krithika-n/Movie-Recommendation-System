...
c1.execute("SELECT distinct movieid from movie_actor ")
for r9 in c1.fetchall():
    ...
    c3.execute("SELECT distinct count(*) from mltags inner join movie_actor on mltags.movieid = movie_actor.movieid where movie_actor.movieid = (%s)", (movid,)) 
    ...
    for r1 in c2.fetchall():
        ...
        c5.execute("SELECT distinct count(mltags.tagid) from mltags inner join movie_actor on mltags.movieid = movie_actor.movieid where mltags.tagid = (%s) and movie_actor.movieid = (%s)",(tag_id,movid) )
...
for i in relatedMovieid[:5]:
    c6.execute("SELECT  distinct moviename from mlmovies where movieid = (%s)",(i,))
    movieName = c6.fetchone()[0]
    print(movieName)
